...
벼농사는 전 세계 메탄(CH₄) 배출의 주요 발생원 중 하나이며, 논을 지속적으로 담수 상태로 유지할 경우 유기물의 혐기성 분해가 증가하여 많은 양의 메탄이 발생합니다. 

Gold Standard AWD(Alternate Wetting and Drying)는 논을 주기적으로 담수와 배수 상태로 관리하여 메탄 배출을 줄이는 대표적인 물 관리 기법입니다. 

하지만 실제 농가에서는 수위 측정, AWD 수행 여부 확인, 데이터 기록 및 보고 과정이 대부분 수작업으로 이루어져 지속적인 관리가 어렵습니다.

이 프로젝트는 IoT 수위 센서, AI 기반 이미지 검증, 웹 기반 MRV(Measurement, Reporting, Verification) 시스템을 활용하여 AWD 관리 과정을 자동화하고 데이터 수집·검증·보고 체계를 구축하는 것을 목표로 합니다. 이를 통해 향후 탄소감축량 산정 및 탄소배출권 제도 연계를 위한 MRV 기반 플랫폼 구축 가능성을 제시하고자 합니다.
