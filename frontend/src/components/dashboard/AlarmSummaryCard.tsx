interface Alert {
  id: number
  node_id: number
  alert_type: 'LOW_WATER' | 'HIGH_WATER'
  message: string
  created_at: string
}
    ...
    <div style={{
      ...
      {recentAlerts.length === 0 ? (
        ...
        recentAlerts.slice(0, 3).map((alert) => (
          <div key={alert.id} style={{ padding: '8px 0', borderTop: '0.5px solid #f0f0f0', fontSize: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
              <span style={{ fontWeight: 500 }}>Node {alert.node_id}</span>
              <span style={{
            ...
            <p style={{ color: '#888' }}>{new Date(alert.created_at).toLocaleString('ko-KR')}</p>
          </div>
        ))
      )}
    </div>
