...
export default function FieldInfo({ fieldName, sensors }: FieldInfoProps) {
  const statusLabel = (status: string) => {
    ...
    if (status === 'OVERFLOODED') return '과담수'
  ...
  const badgeStyle = (status: string) => {
    if (status === 'OVERFLOODED') return { background: '#e3f2fd', color: '#1565c0' }
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '12px', color: '#888', marginBottom: '8px', fontWeight: 500 }}>센서 상태 · {fieldName}</p>
      {sensors.length === 0 ? (
        <p style={{ fontSize: '13px', color: '#aaa' }}>센서 없음</p>
      ) : sensors.map((sensor, i) => (
        <div key={sensor.id} style={{
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          padding: '6px 0', borderBottom: i < sensors.length - 1 ? '0.5px solid #f0f0f0' : 'none',
        }}>
          <div>
